...
watch(headers, () => {
  ...
  if (!last || last.key.trim() !== "" || last.value.trim() !== "") headers.value.push({ key: "", value: "" });
}, { deep: true });

watch(models, () => {
  const last = models.value[models.value.length - 1];
  if (!last || last.model.trim() !== "") models.value.push({ model: "", alias: "" });
}, { deep: true });
      ...
      <div v-if="step === 2" class="space-y-3">
        ...
        <div class="grid gap-2">
          ...
          <div v-for="(row, index) in models" :key="index" class="flex items-center gap-2">
            ...
            <UiButton v-if="row.model.trim()" size="icon-sm" variant="ghost" @click="models.splice(index, 1)">
              ✕
            </UiButton>
          </div>
