...
const { data, error, pending, refresh } = await useAsyncData(
  () => `dashboard-key-analytics-${apiKeyId.value}-${period.value}`,
  () => $client.analytics.byApiKey.query({ apiKeyId: apiKeyId.value, filter: period.value as "15m" | "1h" | "24h" | "7d" | "30d" }),
  { watch: [apiKeyId, period] }
);
...
const selectedApiKeyLabel = computed(() => {
  const apiKey = apiKeys.value.find((item) => item.id === apiKeyId.value);
  return apiKey ? `${apiKey.name ?? "Unnamed key"} (${apiKey.keyPreview})` : apiKeyId.value;
});
...
const stats = computed(() => {
  const totalRequests = summary.value?.requests ?? 0;
  const totalTokens = summary.value?.tokens ?? 0;
  const failedRequests = summary.value?.errors ?? 0;
  const successRate = summary.value?.successRate ?? 0;
  ...
  return [
    ...
    { label: "P50 Latency", value: summary.value?.avgDuration ? formatDuration(summary.value.avgDuration) : "-", hint: "p95 - · p99 -" },
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div class="flex min-w-0 items-center gap-2">
        <UPopover v-model:open="isApiKeyFilterOpen" :content="{ align: 'end' }">
          <button type="button" :disabled="pending || apiKeys.length === 0" class="inline-flex h-8 min-w-0 cursor-pointer items-center justify-between gap-2 overflow-hidden rounded-lg border border-border bg-background px-2.5 text-xs font-medium outline-none transition-all hover:bg-input/50 hover:text-accent-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 sm:h-9 sm:min-w-48 sm:text-sm">
          ...
          <template #content>
            <div class="w-80 max-w-[calc(100vw-2rem)] space-y-1 p-2">
              <NuxtLink to="/dashboard" class="inline-flex h-8 w-full items-center justify-start rounded-md px-2.5 text-xs font-medium hover:bg-accent/50 hover:text-accent-foreground">
                All API keys
              </NuxtLink>
              <NuxtLink
                ...
                :class="[
                  'inline-flex h-8 w-full items-center justify-start rounded-md px-2.5 text-xs font-medium transition-all',
                  apiKey.id === apiKeyId ? 'bg-secondary text-secondary-foreground' : 'hover:bg-accent/50 hover:text-accent-foreground',
                ]"
              >
        ...
        <UPopover v-model:open="isFilterOpen" :content="{ align: 'end' }">
          <button type="button" :disabled="pending" class="inline-flex h-8 min-w-0 cursor-pointer items-center justify-between gap-2 overflow-hidden rounded-lg border border-border bg-background px-2.5 text-xs font-medium outline-none transition-all hover:bg-input/50 hover:text-accent-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 sm:h-9 sm:min-w-48 sm:text-sm">
            <span class="inline-flex min-w-0 items-center gap-1.5">
            ...
            <div class="space-y-2 p-3">
              ...
              <div class="grid grid-cols-1 gap-1">
                <button
                  ...
                  :key="item.value"
                  type="button"
                  :class="['inline-flex h-8 cursor-pointer items-center justify-start rounded-md px-2.5 text-xs font-medium transition-all', period === item.value ? 'bg-secondary text-secondary-foreground' : 'hover:bg-accent/50 hover:text-accent-foreground']"
        ...
        <button type="button" :disabled="pending" class="inline-flex size-8 cursor-pointer items-center justify-center rounded-lg border border-border bg-background text-sm font-medium outline-none transition-all hover:bg-input/50 hover:text-accent-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 sm:size-9" @click="refresh()">
    ...
    <div class="grid gap-3 grid-cols-[repeat(auto-fill,minmax(160px,1fr))]">
      ...
      <template v-else>
        <DashboardStatCard v-for="stat in stats" :key="stat.label" :label="stat.label" :value="stat.value" :hint="stat.hint" />
      </template>
    </div>
