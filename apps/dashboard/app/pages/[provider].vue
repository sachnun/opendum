      ...
      <div v-if="settingsForm" class="min-h-0 flex-1 space-y-4 overflow-y-auto">
        ...
        <div class="grid gap-2">
          ...
          <div v-for="(header, index) in settingsForm.headers" :key="index" class="flex items-center gap-2">
            <input v-model="header.key" class="h-9 flex-1 rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50" placeholder="Header">
            ...
            <UiButton v-if="header.key.trim() || header.value.trim()" size="icon-sm" variant="ghost" @click="settingsForm.headers.splice(index, 1)">
              ✕
            </UiButton>
          </div>
        </div>
        ...
        <div class="grid gap-2 border-t border-border pt-3">
          ...
          <div v-for="(row, index) in settingsForm.models" :key="index" class="flex items-center gap-2">
            ...
            <UiButton v-if="row.model.trim()" size="icon-sm" variant="ghost" @click="settingsForm.models.splice(index, 1)">
              ✕
            </UiButton>
          </div>
        </div>
