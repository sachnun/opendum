...
export function useDashboardApi() {
  ...
  return {
    ...
    accounts: {
      ...
      initiateDeviceAuth: (body: { provider: "copilot" | "codex"; method?: CopilotAuthMethod }) => post<ActionResult<{ deviceCode: string; userCode: string; verificationUrl: string; verificationUrlComplete?: string; codeVerifier?: string }>>(dashboardFetch, "/api/dashboard/accounts/device-auth/initiate", body),
      pollDeviceAuth: (body: { provider: "copilot" | "codex"; deviceCode: string; userCode?: string; codeVerifier?: string; method?: CopilotAuthMethod }) => post<ActionResult<{ status: "pending"; retryAfterSeconds?: number } | { status: "error"; message: string } | { status: "success"; email: string; isUpdate: boolean }>>(dashboardFetch, "/api/dashboard/accounts/device-auth/poll", body),
