...
export function useApi() {
  ...
  return {
    ...
    customProviders: {
      ...
      create: (body: { slug: string; name: string; baseUrl: string; extraHeaders?: Record<string, string> }) => post<ActionResult<CustomProviderCreateResult>>(apiFetch, "/api/dashboard/custom-providers", body),
      update: (body: { slug: string; name?: string; baseUrl?: string; extraHeaders?: Record<string, string>; enabled?: boolean }) => post<ActionResult>(apiFetch, "/api/dashboard/custom-providers/update", body),
      ...
      addModels: (body: { slug: string; models: Array<{ modelId: string; upstream?: string; aliased?: boolean; authless?: boolean; minTier?: string; allowedTiers?: string[]; customFlags?: CustomProviderModelFlags }> }) => post<ActionResult<{ added: number }>>(apiFetch, "/api/dashboard/custom-providers/models", body),
