...
export const summaryAccountsProcedure = protectedProcedure.query(async ({ ctx }) => {
  try {
    const [accounts, providerUsage] = await Promise.all([
      db
        ...
        .from(providerAccount)
        .where(eq(providerAccount.userId, ctx.userId)),
      buildProviderStats(ctx.userId),
    ]);
    const pinnedProviders = await getPinnedProviderKeys(ctx.userId, accounts.map((account) => account.provider));
...
export const byProviderDetailedAccountsProcedure = protectedProcedure.input(z.object({ provider: z.string() })).query(async ({ ctx, input }) => {
  try {
    ...
    const [accountUsage, disabledModelRows, pinnedProviders] = await Promise.all([
      buildAccountStats(ctx.userId, accountIds),
      accountIds.length > 0
        ? db
        ...
        : Promise.resolve([]),
      getPinnedProviderKeys(ctx.userId),
    ]);
...
export const updateAccountProcedure = protectedProcedure
  .input(z.object({ id: z.string(), name: z.string().optional(), isActive: z.boolean().optional() }))
  .mutation(async ({ ctx, input }) => {
    try {
      const [account] = await db.select({ id: providerAccount.id }).from(providerAccount).where(and(eq(providerAccount.id, input.id), eq(providerAccount.userId, ctx.userId))).limit(1);
      ...
      if (Object.keys(updates).length > 0) {
        ...
        if (updates.isActive !== undefined) await invalidateDisabledModelsCache(ctx.userId);
      }
...
export const deleteAccountProcedure = protectedProcedure.input(z.object({ id: z.string() })).mutation(async ({ ctx, input }) => {
  try {
    const [account] = await db.select({ id: providerAccount.id }).from(providerAccount).where(and(eq(providerAccount.id, input.id), eq(providerAccount.userId, ctx.userId))).limit(1);
    if (!account) return { success: false, error: "Account not found" } as const;
...
export const togglePinnedProviderProcedure = protectedProcedure.input(z.object({ providerKey: z.string() })).mutation(async ({ ctx, input }) => {
  ...
  try {
    const [existing] = await db
      ...
      .from(pinnedProvider)
      .where(and(eq(pinnedProvider.userId, ctx.userId), eq(pinnedProvider.providerKey, input.providerKey)))
      .limit(1);
    ...
    await db.insert(pinnedProvider).values({ userId: ctx.userId, providerKey: input.providerKey }).onConflictDoNothing({ target: [pinnedProvider.userId, pinnedProvider.providerKey] });
...
export const setAccountModelEnabledProcedure = protectedProcedure
  .input(z.object({ accountId: z.string(), modelId: z.string(), enabled: z.boolean() }))
  .mutation(async ({ ctx, input }) => {
    try {
      const [account] = await db
        .select({ id: providerAccount.id, provider: providerAccount.provider })
        .from(providerAccount)
        .where(and(eq(providerAccount.id, input.accountId), eq(providerAccount.userId, ctx.userId)))
        .limit(1);
      if (!account) return { success: false, error: "Account not found" } as const;
...
export const errorHistoryAccountsProcedure = protectedProcedure
  .input(z.object({ accountId: z.string(), limit: z.number().int().min(1).max(200).optional() }))
  .query(async ({ ctx, input }) => {
    const [account] = await db.select({ id: providerAccount.id }).from(providerAccount).where(and(eq(providerAccount.id, input.accountId), eq(providerAccount.userId, ctx.userId))).limit(1);
    if (!account) return { success: false, error: "Account not found" } as const;
...
export const resolveErrorsAccountProcedure = protectedProcedure.input(z.object({ accountId: z.string() })).mutation(async ({ ctx, input }) => {
  try {
    const [account] = await db.select({ id: providerAccount.id, status: providerAccount.status }).from(providerAccount).where(and(eq(providerAccount.id, input.accountId), eq(providerAccount.userId, ctx.userId))).limit(1);
    if (!account) return { success: false, error: "Account not found" } as const;
...
export const getAuthUrlAccountsProcedure = protectedProcedure.input(z.object({ provider: z.enum(["antigravity", "gemini_cli", "codex", "kiro"]) })).mutation(async ({ input }) => {
  try {
...
export const exchangeOAuthAccountProcedure = protectedProcedure
  .input(z.object({ provider: z.enum(["antigravity", "gemini_cli", "codex", "kiro"]), callbackUrl: z.string(), state: z.string().nullable().optional(), codeVerifier: z.string().nullable().optional() }))
  .mutation(async ({ ctx, input }) => {
    try {
      ...
      if (input.provider === "antigravity") {
        return await upsertOAuthAccount(ctx.userId, "antigravity", "Antigravity", await antigravityProvider.exchangeCode(parsedUrl.data.code, antigravityRedirectUri));
      }

      if (input.provider === "gemini_cli") {
        return await upsertOAuthAccount(ctx.userId, "gemini_cli", "Gemini CLI", await geminiCliProvider.exchangeCode(parsedUrl.data.code, geminiCliRedirectUri));
      }
      ...
      if (input.provider === "codex") {
        ...
        return await upsertOAuthAccount(ctx.userId, "codex", "Codex", oauthResult, { email: accountId ? `codex-${accountId}` : `codex-${Date.now()}`, accountId });
      }
      ...
      const accountId = oauthResult.accountId || null;
      return await upsertOAuthAccount(ctx.userId, "kiro", "Kiro", oauthResult, { email: accountId ? `kiro-${accountId}` : `kiro-${Date.now()}`, accountId });
    } catch (error) {
...
export const initiateDeviceAuthAccountProcedure = protectedProcedure.input(z.object({ provider: z.enum(["qwen_code", "copilot"]) })).mutation(async ({ input }) => {
  try {
...
export const pollDeviceAuthAccountProcedure = protectedProcedure
  .input(z.object({ provider: z.enum(["qwen_code", "copilot"]), deviceCode: z.string(), codeVerifier: z.string().optional() }))
  .mutation(async ({ ctx, input }) => {
    try {
      ...
      const saved = await upsertOAuthAccount(ctx.userId, provider, label, result, { email });
