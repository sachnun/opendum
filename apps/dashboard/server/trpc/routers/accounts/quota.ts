...
import { fetchCodexQuotaFromApi, type CodexQuotaSnapshot, type CodexRateLimitWindow } from "../../../lib/proxy/providers/codex/quota";
import { copilotProvider } from "../../../lib/proxy/providers/copilot";
import { fetchCopilotUsageFromApi, type CopilotUsageSnapshot } from "../../../lib/proxy/providers/copilot/quota";
import { fetchGeminiCliAccountInfo, geminiCliProvider } from "../../../lib/proxy/providers/gemini-cli/client";
import { fetchGeminiCliQuotaFromApi, type GeminiCliQuotaGroupInfo, type GeminiCliQuotaSnapshot } from "../../../lib/proxy/providers/gemini-cli/quota";
import { kiroProvider } from "../../../lib/proxy/providers/kiro";
import { fetchKiroQuotaFromApi, type KiroQuotaMetric } from "../../../lib/proxy/providers/kiro/quota";
...
export const quotaAccountProcedure = protectedProcedure
  .input(z.object({ provider: quotaProviderSchema, accountId: z.string(), forceRefresh: z.boolean().optional() }))
  .query(async ({ ctx, input }) => {
    try {
      const [account] = await db
        .select()
        .from(providerAccount)
        .where(and(eq(providerAccount.userId, ctx.userId), eq(providerAccount.provider, input.provider), eq(providerAccount.id, input.accountId)))
