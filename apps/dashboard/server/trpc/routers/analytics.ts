...
import { protectedProcedure, router, type ActionResult } from "../init";
...
export const analyticsRouter = router({
  ...
  byApiKey: protectedProcedure
    .input(z.object({ apiKeyId: z.string(), filter: analyticsFilterSchema.optional(), forceRefresh: z.boolean().optional() }))
    .query(async ({ ctx, input }) => {
      const result = await getAnalyticsDataForUser(ctx.userId, input.filter ?? "24h", input.apiKeyId, input.forceRefresh);
      if (!result.success) throw new Error(result.error);
  ...
  usage: protectedProcedure
    .input(z.object({ range: periodSchema.default("24h") }).optional())
    .query(async ({ ctx, input }) => {
      ...
      try {
        return await db
          ...
          .where(and(eq(usageLog.userId, ctx.userId), gte(usageLog.createdAt, resolvedFilter.data.startDate), lte(usageLog.createdAt, resolvedFilter.data.endDate)))
