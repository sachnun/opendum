  ...
  options: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const providerAccounts = await db
        ...
        .where(and(eq(providerAccount.userId, ctx.userId), eq(providerAccount.isActive, true)))
      ...
      const apiKeyOptions = proxyBaseUrl
        ? (await db
            ...
            .from(proxyApiKey)
            .where(and(eq(proxyApiKey.userId, ctx.userId), eq(proxyApiKey.isActive, true)))
