...
app.get("/v1", (c) => writeOpenAIError(c, 404, {
...
async function bootstrap() {
    ...
    app.get("/v1/models", async (c) => {
        ...
        const availability = await authService.getAccountModelAvailabilityWithSharing(userId, Boolean(authResult.roamingEnabled));
        const apiKeyModelSet = new Set((authResult.modelAccessList ?? []).map((m) => registry.resolveAlias(m)));
        const enabled = allModels.filter((item) => {
            const canonical = registry.resolveAlias(item.id);
            if (disabledSet.has(canonical))
                return false;
            const usableOwned = authService.isModelUsableByAccounts(canonical, availability);
            const usableShared = authResult.roamingEnabled &&
                authService.isModelUsableBySharedAccounts(canonical, availability);
            if (!usableOwned && !usableShared)
                return false;
            if (authResult.modelAccessMode === "whitelist") {
                if (!apiKeyModelSet.has(canonical))
                    return false;
            }
