...
func transformOpenAIToAnthropic(openAI map[string]any, model string) map[string]any {
	...
	usage := map[string]any{"input_tokens": counts.inputTokens, "output_tokens": counts.outputTokens}
	if counts.cachedTokens > 0 {
		usage["cache_read_input_tokens"] = counts.cachedTokens
	}
	if counts.cacheWriteTokens > 0 {
		usage["cache_creation_input_tokens"] = counts.cacheWriteTokens
	}
