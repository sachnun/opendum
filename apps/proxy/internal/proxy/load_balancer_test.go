...
func TestAccountHealthStateViewReportsStatusForLookupKeys(t *testing.T) {
	now := time.Date(2026, 5, 13, 12, 0, 0, 0, time.UTC)
	state := accountHealthState{
		account: appdb.GetAccountHealthStateRow{ID: "acct", Status: "active"},
		rows: []appdb.ProviderAccountModelHealth{
			{ID: "h1", ProviderAccountID: "acct", Model: "canonical", ConsecutiveErrors: 2, Status: "degraded"},
			{ID: "h2", ProviderAccountID: "acct", Model: "other", ConsecutiveErrors: 0, Status: "active"},
		},
	}

	view := state.view(now, []string{"alias-of-canonical", "canonical"})
	if view.coolingDown {
		t.Fatal("coolingDown = true, want false")
	}
	if !view.hasHealth || view.status != "degraded" {
		t.Fatalf("view = %+v, want degraded with health", view)
	}
}

func TestAccountHealthStateViewTreatsMissingModelHealthAsUsable(t *testing.T) {
	now := time.Date(2026, 5, 13, 12, 0, 0, 0, time.UTC)
	state := accountHealthState{
		account: appdb.GetAccountHealthStateRow{ID: "acct", Status: "active"},
		rows:    []appdb.ProviderAccountModelHealth{{ID: "h1", ProviderAccountID: "acct", Model: "other"}},
	}

	view := state.view(now, []string{"canonical"})
	if view.hasHealth {
		t.Fatalf("view = %+v, want no health for unknown model", view)
	}
	if view.coolingDown {
		t.Fatal("coolingDown = true, want false")
	}
...
func TestAccountHealthStateViewCoolingDownOnActiveCooldown(t *testing.T) {
	now := time.Date(2026, 5, 13, 12, 0, 0, 0, time.UTC)
	disabledUntil := now.Add(5 * time.Minute)
	state := accountHealthState{account: appdb.GetAccountHealthStateRow{ID: "acct", Status: "failed", DisabledUntil: &disabledUntil}}

	if view := state.view(now, nil); !view.coolingDown {
		t.Fatalf("view = %+v, want coolingDown while disabledUntil is in the future", view)
	}
}

func TestAccountHealthStateViewCoolsDownAtThreshold(t *testing.T) {
	now := time.Date(2026, 5, 13, 12, 0, 0, 0, time.UTC)
	state := accountHealthState{
		account: appdb.GetAccountHealthStateRow{ID: "acct", Status: "active"},
		rows:    []appdb.ProviderAccountModelHealth{{ID: "h1", ProviderAccountID: "acct", Model: "canonical", ConsecutiveErrors: accountCooldownUnhealthyThreshold, Status: "degraded"}},
	}

	if view := state.view(now, []string{"canonical"}); !view.coolingDown {
		t.Fatalf("view = %+v, want coolingDown at the unhealthy threshold", view)
	}
}

func TestAccountHealthStateViewAppliesCooldownRecovery(t *testing.T) {
	now := time.Date(2026, 5, 13, 12, 0, 0, 0, time.UTC)
	expired := now.Add(-time.Minute)
	state := accountHealthState{
		account: appdb.GetAccountHealthStateRow{ID: "acct", Status: "failed", DisabledUntil: &expired},
		rows:    []appdb.ProviderAccountModelHealth{{ID: "h1", ProviderAccountID: "acct", Model: "canonical", ConsecutiveErrors: 10, Status: "degraded"}},
	}

	view := state.view(now, []string{"canonical"})
	if view.coolingDown {
		t.Fatal("coolingDown = true, want false once the cooldown expired")
	}
	if view.status != "degraded" {
		t.Fatalf("status = %q, want degraded after 30%% recovery", view.status)
	}
}
