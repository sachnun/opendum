...
- Model registry refresh: `pnpm models:refresh`. It runs many provider scripts, mutates `models/**`, and may need network/provider availability.
...
- Dashboard provider implementations are in `apps/dashboard/server/lib/proxy/providers`. Go proxy currently supports API-key providers; README notes OAuth-provider parity is incomplete there.
...
- Proxy loads `.env` and `apps/proxy/.env`; defaults `PORT=4001`, `HOST=0.0.0.0`. Set `MODELS_DIR` only if repo-layout autodetect fails.
...
- Proxy Railway build is `mkdir -p dist && cd apps/proxy && go build -o ../../dist/proxy ./cmd/proxy`; watch paths include `apps/proxy/**`, `models/**`, root `package.json`, and `railway/proxy.json`.
