...
import { AntigravityProvider } from "./antigravity/client.js";
import { ClineProvider } from "./cline/client.js";
import { CodexProvider } from "./codex.js";
import { KiroProvider } from "./kiro.js";
import { OpencodeProvider } from "./opencode/client.js";
import { PerchProvider } from "./perch.js";
import { QoderProvider } from "./qoder/client.js";
...
export class ProviderRegistry {
    ...
    constructor(private registry: ModelRegistry) {
        ...
        this.registerBuiltInCustomProviders();
    }

    private registerBuiltInCustomProviders(): void {
        for (const provider of [
            new AntigravityProvider(this.registry),
            new ClineProvider(this.registry),
            new CodexProvider(this.registry),
            new KiroProvider(this.registry),
            new OpencodeProvider(this.registry),
            new PerchProvider(this.registry),
            new QoderProvider(this.registry),
            new WorkersAIProvider(this.registry),
        ]) {
    ...
    public async autoDiscover(providersDir?: string): Promise<void> {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                if (
                    instantiated &&
                    typeof instantiated.makeRequest === "function"
                ) {
                    this.register(instantiated);
                }
            } catch (err) {
                console.warn(
                    `Failed to auto-load provider from ${targetFile}:`,
                    err,
                );
            }
        }
